#!/bin/bash
# Exp5 - FashionMNIST: m=4 attackers (vs exp4 m=8)
COMMON="--model cnn --dataset FashionMNIST --lr 0.02 --ld True --exp 5 --device cuda --device_id 0"
filters="avg multi-krum median trmean flame"

echo "=== Starting none ==="
for filter in ${filters}; do
    echo "Running: filter=${filter}, mp=none"
...
for filter in ${filters}; do
    echo "Running: filter=${filter}, mp=sign_flip"
...
echo "=== Starting LIE ==="
for filter in ${filters}; do
    echo "Running: filter=${filter}, mp=LIE"
...
echo "=== Starting MPAF ==="
for filter in ${filters}; do
    echo "Running: filter=${filter}, mp=MPAF"
...
for filter in ${filters}; do
    echo "Running: filter=${filter}, mp=CAMP_clipping_v8"
