...
def separate_data(data, num_clients, num_classes, niid=False, balance=False, partition=None, class_per_client=None, p=0.5):
    ...
    if partition == 'pat':
        ...
        idx_for_each_group = [[] for _ in range(num_group)]
        # for client in range(num_clients):
        #     client_groups[client % num_classes].append(client)

        for i in range(num_classes):
            # 按照概率 p 分配标签样本
            idx_class = idx_for_each_class[i]
            num_samples = len(idx_class)

            # 计算分配到第 l 组的样本数量
            group_assignment = np.random.choice(
                num_group, size=num_samples, p=[p if j == i else (1 - p) / (num_group - 1) for j in range(num_group)]
            )
            
            # 分配到相应的组
            for group in range(num_group):
                group_indices = idx_class[group_assignment == group]
                idx_for_each_group[group].extend(group_indices)

        # 3. 将每组分到的样本平等分配给组内的客户端
        client_for_each_group = [[] for _ in range(num_group)]
        for client in range(num_clients):
            client_for_each_group[client % num_group].append(client)
            if client not in dataidx_map.keys():
                dataidx_map[client] = []
        
        for group in range(num_group):
            idx_in_group = idx_for_each_group[group]
            clients_in_group = client_for_each_group[group]

            assignment = np.random.choice(
                clients_in_group, size=len(idx_in_group)
            )

            for client, idx in zip(assignment, idx_in_group):
                dataidx_map[client].append(idx)
